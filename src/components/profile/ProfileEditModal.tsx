...
export const ProfileEditModal: React.FC<ProfileEditModalProps> = ({ profile, isOpen, onClose, onSaved }) => {
  ...
  const handleAddInterest = () => {
    ...
    if (!currentList.includes(interestInput.trim())) {
      setFormData(prev => ({ ...prev, interests: [...currentList, interestInput.trim()] }));
    }
    setInterestInput('');
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-5 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h2 className="text-lg font-bold text-slate-100">Edit Anonymous Profile</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-white text-xl">✕</button>
        </div>

        {/* 0-Byte Storage Avatar Selector */}
        <AvatarSelector 
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100 focus:outline-none focus:border-purple-500 resize-none" 
              maxLength={150} 
            />
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-2 mb-2">
              <input 
                ...
                className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100 focus:outline-none focus:border-purple-500" 
              />
              <button 
                type="button" 
            ...
            <div className="flex flex-wrap gap-1">
              {(formData.interests || []).map(item => (
                <span key={item} className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded-lg flex items-center space-x-1">
                  <span>{item}</span>
                  <button type="button" onClick={() => handleRemoveInterest(item)} className="text-red-400 hover:text-red-300 font-bold">×</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="pt-2 flex justify-end space-x-2">
            ...
            <button 
              type="submit" 
              disabled={isSaving} 
              className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-semibold disabled:opacity-50"
            >
